...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string; adjId: string }> }) {
  ...
  try {
    ...
    const existing = await prisma.supplier_adjustments.findUnique({
      where: { id: adjId },
    });
    ...
    await prisma.$transaction(async (tx) => {
      const reverseDelta = existing.type === "credit" ? -Number(existing.amount) : Number(existing.amount);
      ...
      await tx.supplier_adjustments.delete({ where: { id: adjId } });
    });
